...
const CHARACTER_PERSONAS: Record<string, string> = {
  ...
  panda:
    "You are Panda Buddy, a warm and encouraging companion. Celebrate progress and keep learners motivated.",
};
...
const PLAN_CAPABILITIES: Record<string, string> = {
  ...
  business:
    "You help with everything in Pro, plus: advanced linguistic analysis, industry terminology (legal, medical), business writing, team learning, workflow optimization.",
};
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: accessRow } = await (
      supabase.rpc as unknown as (
        ...
        data: { current_plan?: string; ai_limit?: number; ai_used_today?: number } | null;
      }>
    ...
    if (limit !== -1) {
      ...
      if (used >= limit) {
        await (
          ...
          _event_type: "ai_limit_block",
          _plan: plan,
          _details: { used, limit, character: data.character },
        });
        return {
          ok: false as const,
          error: "limit_reached",
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 402) {
        return {
          ok: false as const,
          error: "credits",
          message: "AI credits exhausted. Please add credits in workspace usage settings.",
        };
      }
